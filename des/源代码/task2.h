...
static void  DES_Efun(int  chartobit[64], int  key[64], int output[64])
{
...
void getbits(char c,int bits[8]){
  for(int i=0;i<8;i++)
    {
        //  data+=c&(0x01<<i);
        if ( ( c >>(i-1) ) & 0x01 == 1 )
...
int mainFuntion2(char Plaintext[8],char Secret_key[8],int changeSum)
{
    srand(time(NULL));
...
int tempOutput[64] = {0};
    ...
    for(int i=0;i<10;i++){//随机获取明文改变的位置，改变明文10次，计算密文位数改变平均值
     int randNum =  rand()%64;
     for(int i=0;i<64;i++){tempPlaintextbits[i] = PlaintextBits[i];}//对明文bits重新赋值
     for(int j=0;j<changeSum;j++){//对明文的changeSum位进行改变
        if(tempPlaintextbits[(randNum+j)%64]==0)
            tempPlaintextbits[(randNum+j)%64] = 1;
        else tempPlaintextbits[(randNum+j)%64] = 0;
     }

     DES_Efun(tempPlaintextbits, Secret_keyBits, tempOutput);//获取明文改变后的密文
